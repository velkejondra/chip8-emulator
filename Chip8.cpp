...
bool operator>(const OpcodeMethod left, unsigned short right) {
  for (int i = 0; i < ceil((double)right/0xF); ++i) {
    char curbyte = (right >> (0xF * i))  & 0xF;
    char CurOpCodeByte = (left.MyOpcode >> (0xF * poradi[i]))  & 0xF;
    if (CurOpCodeByte == curbyte) continue;
    if (CurOpCodeByte > curbyte) return true;
    if (CurOpCodeByte < curbyte) return false;
  }
  return false;
}
bool operator==(const OpcodeMethod left, unsigned short right) {
